...
def test_dev_seed_creates_fixed_demo_accounts_with_login_credentials(tmp_path):
    ...
    with session_factory() as session:
        ...
        assert [(user.email, user.role) for user in users] == [
            ...
            ("demo.blocking@apps.ipb.ac.id", UserRole.student),
            ("demo.staff@ipb.ac.id", UserRole.staff),
...
def test_dev_seed_creates_frontend_reservation_tracer_data(tmp_path):
    ...
    with session_factory() as session:
        ...
        blocking_student = session.scalar(select(User).where(User.email == "demo.blocking@apps.ipb.ac.id"))
        ...
        blocking_reservations = session.scalars(
            select(Reservation).where(Reservation.student_id == blocking_student.id)
        ).all()

    assert demo_student_reservations == []
    assert {reservation.status for reservation in blocking_reservations} >= {
...
def test_dev_seed_is_idempotent_for_seeded_database_rows(tmp_path):
    ...
    with session_factory() as session:
        ...
        assert session.scalar(select(func.count()).select_from(FacilityStaffAssignment)) == 1
