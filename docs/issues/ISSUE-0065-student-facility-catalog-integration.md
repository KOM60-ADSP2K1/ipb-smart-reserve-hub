...
- 2026-05-13: Updated `frontend/tests/e2e/student-facility-catalog.spec.ts` to mock backend discovery endpoints and refreshed catalog desktop/mobile screenshots for the intentional category/sort normalization.
  Verification passed: `npm test -- --run src/pages/student/StudentFacilityCatalogPage.test.tsx`, `npm run typecheck`, `npx playwright test backend/tests/e2e/student-facility-catalog.spec.ts --update-snapshots`, and `npx playwright test backend/tests/e2e/student-facility-catalog.spec.ts`.
