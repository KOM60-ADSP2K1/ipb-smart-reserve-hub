...
- 2026-05-13: Verification: `npm test -- --run src/pages/staff/StaffFacilityPages.test.tsx`, `npm run typecheck`, and `npx playwright test backend/tests/e2e/staff-edit-facility.spec.ts` pass. `npm run lint` still fails on pre-existing unrelated Student/auth lint errors; no Staff Facility files are reported.
